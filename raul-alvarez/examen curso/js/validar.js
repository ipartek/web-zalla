...
function validar() {
    ...
    if ((formulario.precio.value <= 1) || (isNaN(formulario.precio.value))) {
        toRemove = document.getElementById('errorPrecio');
        if (toRemove) {
            toRemove.remove();
        }
        txtErrorS = document.getElementById("precio");
    ...
    } else if (parseInt(formulario.stock.value) > 1 && muestraPrecio == 1) {
    ...
    if ((formulario.stock.value <= 1) || (isNaN(formulario.stock.value))) {
